...
function NicknameEditForm() {
  ...
  const [nickname, onChageNick, setNick] = useInput('')
  const [descrip, onChageDescrip, setDesc] = useInput('')
  ...
  return (
    <div>
      <Form style={style} onFinish={onSubmit}>
        ...
        <Input.TextArea placeholder='소개 글' value={descrip} onChange={onChageDescrip} />
        <Button type='primary' htmlType='submit' disabled={nickname || descrip ? '' : 'disabled'} >change</Button>
      </Form>
    </div>
  )
}
